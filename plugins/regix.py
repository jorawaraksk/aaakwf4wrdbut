        ...
        try:
          ...
          async for message in iter_messages(client, chat_id=sts.get("FROM"), limit=sts.get("limit"), offset=sts.get("skip"), filters=filter, max_size=max_size):
                ...
                if forward_tag:
                   ...
                   if message.photo and "photo" in datas['filters']:
                       new_caption = None
...
async def msg_edit(msg, text, button=None, wait=None):
    try:
        ...
        # Avoid crashing if the content is the same
        return
    except FloodWait as e:
        if wait:
            await asyncio.sleep(e.value)
...
async def edit(user, msg, title, status, sts):
   ...
   if status in ["cancelled", "completed"]:
      button.append([InlineKeyboardButton('• ᴄᴏᴍᴘʟᴇᴛᴇᴅ ​•', url='https://t.me/iam_here_wru')])
   else:
